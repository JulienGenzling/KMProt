...
def get_obj(dataset, kernel_params, fitter_params, verbose, cache):
    if kernel_params["name"] == "spectrum":
        kernel = MultiSpectrumKernel(
            dataset, **kernel_params, verbose=verbose, cache=cache
        )
    if kernel_params["name"] == "mismatch":
        kernel = MismatchKernel(dataset, **kernel_params, verbose=verbose, cache=cache)
    else:
...
def get_obj_ensemble(dataset, kernel_configs, fitter_params, verbose, cache):
    kernel = WeightedSumKernel(dataset, kernel_configs, verbose=verbose, cache=cache)
    fitter = get_fitter(fitter_params)
    return fitter, kernel
    ...
    if not ensemble:
        ...
        for phi_test in tqdm(test_phis, desc=f"Predicting fold {datafold}"):
            ...
            for weight, phis in kernel.phis.items():
                phi_test_norm = np.sqrt(kernel.dot(phi_test[weight], phi_test[weight]))
                phis_norms = np.array(
